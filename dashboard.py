...
def preparar_grafico(df, coluna):
    ...
    ordem = (
        dados.sort_values(by="entregas_atrasadas", ascending=False)[coluna]
        .unique()
        .tolist()
    )

    dados[coluna] = pd.Categorical(dados[coluna], categories=ordem, ordered=True)
...
df["entrega_atrasada"] = (
    df["atraso"] > 0
)  # adicionar campo booleano de entrega atrasada
...
df_filtrado = df[
    (df["transportadora"].isin(sel_transp)) & (df["regiao"].isin(sel_regiao))
]
...
with kpi1:
    ...
    percentual = int((atrasadas / total) * 100) if total > 0 else 0

    st.metric("Entregas atrasadas", f"{atrasadas}/{total} ({percentual:.0f}%)")
    st.progress(percentual)

with kpi2:
    media_atrasos = df_filtrado.loc[df_filtrado["entrega_atrasada"], "atraso"].mean()
...
with kpi3:
    ...
    st.metric("Maior atraso", f"{maior_atraso} dias")
...
with col1:
    ...
    dados_grafico = preparar_grafico(df_filtrado, "transportadora")
...
with col2:
    ...
    dados_grafico = preparar_grafico(df_filtrado, "regiao")
...
st.dataframe(
    df_ranking.style.apply(colorir_atrasadas, axis=1),
    hide_index=True,
    column_config={
